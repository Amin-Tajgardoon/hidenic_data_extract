...
def get_loinc_codes(mimic_mapping_filename):
    # Read in the second level mapping of the itemids
    var_map = DataFrame.from_csv(mimic_mapping_filename, index_col=None) #.fillna('').astype(str)
    # var_map = var_map.ix[(var_map['LEVEL2'] != '') & (var_map.COUNT>0)]
    # var_map = var_map.ix[(var_map.STATUS == 'ready')]
    # var_map.ITEMID = var_map.ITEMID.astype(int)
    loinc_codes = var_map.LOINC_CODE[(var_map.LEVEL2.notna() & var_map.LOINC_CODE.notna())]
    return loinc_codes
...
def save_numerics(
    ...
    # TODO: no variable map for HIDENIC, skip below
    ...
    # TODO: HIDENIC doesn't have var_map et, skip line below
    # X = X.join(var_map).join(I).set_index(['label', 'LEVEL1', 'LEVEL2'], append=True)

    # TODO: assume uniform units in all HIDENIC tables, skip for now
    # standardize_units(X, name_col='LEVEL1', inplace=True)

    # TODO: no ranges defines for HIDENIC, skip for now
    if apply_var_limit > 0: 
        X = apply_variable_limits(X, var_ranges, 'LEVEL2')

    # TODO: no groupings available for HIDENIC, skip for now
    # group_item_cols = ['LEVEL2'] if group_by_level2 else ITEM_COLS
    group_item_cols = ['itemid']
    ...
    if group_by_level2:
        ...
        # TODO: columns have two levels for HIDENIC: "itemid" and "Aggregation function", slice the column index accordingly
        # X.loc[:, idx[:,:,:,:, 'count']] = X.loc[:, idx[:,:,:,:, 'count']].fillna(0)
        X.loc[:, idx[:, 'count']] = X.loc[:, idx[:, 'count']].fillna(0)
...
if __name__ == '__main__':
    ...
    ap.add_argument('--group_by_level2', action='store_true', dest='group_by_level2',
    ...
    # TODO: mapping and ranges are not available for all HIDENIC, if prepared, use to group variables or remove outliers
    ...
    # TODO: create similar schema for HIDENIC: static schema fixed for HIDENIC
    ...
    # TODO: define schema for HIDENIC
    ...
    if (args['extract_pop'] == 0 | (args['extract_pop'] == 1) ) & isfile(os.path.join(outPath, static_filename)):
        ...
        select distinct i.subject_id, i.hadm_id, i.icustay_id,
            i.gender, i.age, i.dod AS deathtime, i.race,
            i.hospstay_seq, i.los_icu,
            i.admittime, i.dischtime,
            i.intime, i.outtime,
            CASE when  i.dod between i.intime and i.outtime THEN 1 ELSE 0 END AS mort_icu,
            CASE when  i.dod between i.admittime and i.dischtime THEN 1 ELSE 0 END AS mort_hosp,
            adm.icu_category AS first_careunit
            -- i.hospital_expire_flag, a.insurance
          FROM icustay_detail i
          -- INNER JOIN admissions a ON i.hadm_id = a.hadm_id
          INNER JOIN admit_discharge adm ON i.icustay_id = adm.icuid
          WHERE adm.icu_category NOT like 'NICU'
    ...
    if (args['extract_numerics'] == 0 | (args['extract_numerics'] == 1) ) & isfile(os.path.join(outPath, dynamic_hd5_filename)):
        ...
        ########
        # Step 1) Get the set of icu IDs we want for the patients we've identified!
        icuids_to_keep = get_values_by_name_from_df_column_or_index(data, 'icustay_id')
        icuids_to_keep = set([str(s) for s in icuids_to_keep])
        data = data.copy(deep=True).reset_index().set_index('icustay_id')

        # TODO: create variable mapping for HIDENIC vitals and labs
        # Select out SID, TIME, ITEMID, VALUE form each of the sources!
        var_map = None
        # var_map = get_variable_mapping(mimic_mapping_filename)

        # TODO: define ranges for vitals and labs, otherwise set to None and skip outlier removal
        var_ranges = None
        ...
        # my version that didn't work because LOINC codes were corrupted
        # labitems_to_keep = get_loinc_codes(mimic_mapping_filename)
        # labitems_to_keep = set([ str(i) for i in labitems_to_keep ])
        ...
        # TODO: create items table for HIDENIC, for now use LOINC_CODE and VitalName as item IDs
        # TODO: add Flag for labs, i.e., Flag in (High, Low, Normal)
        query = \
        """
        select i.subject_id, i.hadm_id, i.icustay_id, 
        v."EventDate" AS charttime, 
        v."RollupName" AS itemid,
        v."RollupVal" AS value, 
        v."RollupUnit" AS valueuom
        FROM icustay_detail i
        INNER JOIN vitals v ON i.hadm_id = v."PatientVisitID"
        where i.icustay_id in ({icuids})
        -- and v.itemid in (chitem)
          and v."EventDate" between intime and outtime
        -- and c.error is distinct from 1
          and v."RollupVal" is not null

        UNION ALL

        select distinct i.subject_id, i.hadm_id, i.icustay_id, 
            l."EventDate" AS charttime, 
            l."LOINC_CODE" AS itemid,
            l."EventValue" AS value,
            l."EventUnit" AS valueuom
        FROM icustay_detail i
        INNER JOIN labs l ON i.hadm_id = l."PatientVisitID"
        where i.icustay_id in ({icuids})
          -- and l.itemid in (lbitem)
          and l."EventDate" between (intime - interval '6' hour) and outtime -- why 6 hours?
          and l."EventValue" > 0 -- lab values cannot be 0 and cannot be negative and cannot be null
        ;
    ...
    if ( (args['extract_codes'] == 0) or (args['extract_codes'] == 1) ) and isfile(os.path.join(outPath, codes_hd5_filename)):
        ...
        # TODO: may want to use only icd9 codes with Seq=1
        query = \
        """
        SELECT
            i.icustay_id, d."PatientID" AS subject_id, d."PatientVisitID" AS hadm_id,
            array_agg(d."IcdCpt" ORDER BY "Seq" ASC) AS icd9_codes
        FROM icd_cpt d INNER JOIN icustay_detail i
        ON i.hadm_id = d."PatientVisitID" AND i.subject_id = d."PatientID"
        WHERE d.type_derived = 'ICD9CM_DX' AND d."PatientVisitID" IN ({hadm_ids}) AND "Seq" IS NOT NULL
        GROUP BY i.icustay_id, d."PatientID", d."PatientVisitID"
        ;
    ...
    # TODO: matrix I is not supported in HIDENIC yet, skip for now
    ...
    if C is not None:
        print 'ICD9 names : ', ",".join(icd_names)
